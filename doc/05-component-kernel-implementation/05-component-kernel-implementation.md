...
My component works off of the OSU Queue data structure. This is due to the fact that a playlist works very similarly to a Queue in that it is a FIFO (First-in-first-out) structure. This is due to the fact that songs are always added to the end of the playlist, and removed from the front. Since the OSU Queue data structure provides a defined and efficient format, that we have used extensively, it can be used to make the Music Playlist component. Similar to OSU's structue, the representation shoudl not have null entries, and each entry should be a non-empty String.
